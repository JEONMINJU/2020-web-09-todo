...
function addHTML(k, v) {
	...
	html += '<button class="btn btn-sm bt-delete" onclick="onDelete(\''+k+'\');"><i class="fa fa-trash"></i> </button>';
	html += '</li>';
	...
	return $li; //이건 의미는 없지만 혹시 작업할 거 있으면 써먹어라,
}
...
function onAdd(r) {
	if(!r.val().checked) addHTML(r.key, r.val()); //checked가 있으면 addHTML해달라.
	// $(".add-wrap")[0].reset(); <- reset은 폼을 초기화해주는 자바스크립트 함수이고, 제이쿼리의 [0]데이터가 자바스크립트 객체다.
...
auth.onAuthStateChanged(onAuthChg); //auth의 상태가 변화되면 onAuthChg를 실행시켜주는 로그인상태 감시자, 왓처!!!
moment.locale('ko');
